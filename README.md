...
```powershell
git clone https://github.com/happy-cherry27/foreign-trade-lead-demo.git
cd foreign-trade-lead-demo
...
飞书同步已经提供 OpenAPI 适配和本地演示兜底；配置环境变量后可写入真实飞书多维表格。邮箱和 n8n 侧通过通用 webhook 接入，避免把核心系统绑定到单一邮箱服务。
